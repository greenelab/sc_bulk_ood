...
def get_transform_vec_VAE(X_full, meta_df, curr_samp, encoder, decoder, batch_size):
...
def calc_VAE_perturbation(X_full, meta_df, encoder, decoder, 
    ...
    mu_slack, z_slack = encoder.predict(X_pert, batch_size=batch_size)
    decoded_1_1 = decoder.predict(z_slack, batch_size=batch_size)
    ...
    # get the CTRL
    mu_slack, z_slack = encoder.predict(X_temp, batch_size=batch_size)
    decoded_0_0 = decoder.predict(z_slack, batch_size=batch_size)

    proj_dic = {}
    for curr_samp in np.unique(ctrl_test_meta_df.sample_id):
        proj_vec = get_transform_vec_VAE(X_full, meta_df, curr_samp, encoder, decoder, batch_size)
        proj_dic[curr_samp] = proj_vec


    decoded_0_1 = np.copy(decoded_0_0)
    for curr_idx in range(decoded_0_0.shape[0]):
        curr_samp = ctrl_test_meta_df.iloc[curr_idx].sample_id
        proj_vec = proj_dic[curr_samp]
        decoded_0_1[curr_idx] = decoded_0_0[curr_idx] + proj_vec
...
def calc_PCA_perturbation(X_full, meta_df, scaler, fit):
    ...
    decoded_1_1 = fit.transform(X_pert)
    ...
    decoded_0_0 = fit.transform(X_temp)
    decoded_0_1 = np.copy(decoded_0_0)

    for curr_idx in range(decoded_0_0.shape[0]):
        curr_samp = ctrl_test_meta_df.iloc[curr_idx].sample_id
        proj_vec = get_transform_vec_PCA(X_full, meta_df, curr_samp, fit)
        decoded_0_1[curr_idx] = decoded_0_0[curr_idx] + proj_vec


    decoded_1_1 = fit.inverse_transform(decoded_1_1)
    decoded_1_1 = scaler.inverse_transform(decoded_1_1)
...
def calc_CVAE_perturbation(X_full, meta_df, encoder, decoder, 
    ...
    # get the original perturbation output
    mu_slack = encoder.predict([X_pert, label_1hot_temp[idx_stim_test], perturb_1hot_temp[idx_stim_test]], batch_size=batch_size)
    z_concat = np.hstack([mu_slack, label_1hot_temp[idx_stim_test], perturb_1hot_temp[idx_stim_test]])
    decoded_1_1 = decoder.predict(z_concat, batch_size=batch_size)
    decoded_1_1 = scaler.inverse_transform(decoded_1_1)
    decoded_1_1 = decoded_1_1[range(len_stim_test),]
    ...
    perturbed_code = perturb_1hot_temp[idx_stim_test]
    perturbed_code = np.repeat(perturbed_code, 5, axis=0)


    mu_slack = encoder.predict([X_temp, label_1hot_temp[idx_ctrl_test], perturbed_code[idx_ctrl_test]], batch_size=batch_size)
    z_concat = np.hstack([mu_slack, label_1hot_temp[idx_ctrl_test], perturbed_code[idx_ctrl_test]])
    decoded_0_1 = decoder.predict(z_concat, batch_size=batch_size)
    decoded_0_1 = scaler.inverse_transform(decoded_0_1)
    decoded_0_1 = decoded_0_1[range(len_ctrl_test),]

    mu_slack = encoder.predict([X_temp, label_1hot_temp[idx_ctrl_test], perturb_1hot_temp[idx_ctrl_test]], batch_size=batch_size)
    z_concat = np.hstack([mu_slack, label_1hot_temp[idx_ctrl_test], perturb_1hot_temp[idx_ctrl_test]])
